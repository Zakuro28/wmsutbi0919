                        ...
                        <ul class="list-group">
                            <li class="list-group-item list-group-item-action">
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input" type="checkbox" id="brand_all" value="" <?= !is_array($brand_filter) && $brand_filter =='all' ? 'checked' : '' ?>>
                            ...
                            ?>
                                <li class="list-group-item list-group-item">
                                    <div class="custom-control custom-checkbox">
                                        <input class="custom-control-input brand_filter" type="checkbox" id="brand_<?= $row['id'] ?>" value="<?= $row['id'] ?>" <?= ((is_array($brand_filter) && in_array($row['id'],$brand_filter)) || (!is_array($brand_filter) && $brand_filter =='')) ? 'checked' : '' ?>>
                ...
                <div class="card card-outline shadow card-primary rounded-0">
                    ...
                    <div class="card-body">
                        <ul class="list-group">
                            <li class="list-group-item list-group-item-action">
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input" type="checkbox" id="category_all" value="" <?= !is_array($category_filter) && $category_filter =='all' ? 'checked' : '' ?>>
                            ...
                            ?>
                                <li class="list-group-item list-group-item">
                                    <div class="custom-control custom-checkbox">
                                        <input class="custom-control-input category_filter" type="checkbox" id="category_<?= $row['id'] ?>" value="<?= $row['id'] ?>" <?= ((is_array($category_filter) && in_array($row['id'],$category_filter)) || (!is_array($category_filter) && $category_filter =='')) ? 'checked' : '' ?>>
                        ...
                        <a class="col px-1 py-2 text-decoration-none text-dark product-item" href ="./?p=products/view_product&id=<?= $row['id'] ?>">
                            <div class="card rounded-0 shadow">
                                <div class="product-img-holder overflow-hidden position-relative">
                                    ...
                                    <span class="position-absolute price-tag rounded-pill text-light px-3">
                                        <i></i> <b>₱ <?= number_format($row['price'],2) ?></b>
                                    </span>
                                </div>
    ...
    $(function(){
        ...
        $('#brand_all').change(function(){
            if($(this).is(':checked') ==true){
                ...
                $('.brand_filter').prop("checked",false).trigger('change')
        }  
        })
